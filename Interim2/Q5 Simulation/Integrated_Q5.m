...
% Limit to 0–120 deg
idx_sim = (theta_sim_deg >= 0) & (theta_sim_deg <= 120);
...
% --- Analytical already uses:
% in = 0:120;
% Ax, Ay (position)
% Ax_dot, Ay_dot (velocity)
...
figure
plot(in, Ax_dot, '--', 'Color', [0.6 0 0.6], 'LineWidth', 2) % analytical Vx
hold on
plot(in, Ay_dot, '--', 'Color', [0 0.6 0], 'LineWidth', 2)   % analytical Vy

plot(theta_sim_deg, vx_sim, 'Color', [1 0.4 1], 'LineWidth', 2) % sim Vx (pink)
plot(theta_sim_deg, vy_sim, 'Color', [0.5 1 0], 'LineWidth', 2) % sim Vy (lime)
